...
Two model families (SVM and XGBoost) are evaluated across two experimental settings:
...
Each experiment includes baseline models, SHAP-driven refined models, 5-fold cross-validation, threshold tuning, and full biomarker analysis.
...
After cross-platform gene symbol harmonisation, **566 common genes** were identified across Western and Indian datasets. All cross-population models operate on this shared feature space.
...
Raw Gene Expression Data
  ...
  SMOTE                   synthetic oversampling on Western training set only
         ...
         ├──────────────────────────────────────┐
         ▼                                      ▼
  Baseline Model                       Biomarker Discovery
  (all genes)                          Random Forest + SHAP
         │                                      │
         │                             Top 20-30 SHAP genes
         │                                      │
         └─────────────────┬────────────────────┘
                           ▼
                  Refined Model            SVM (RBF) or XGBoost
                           │
                           ▼
                  5-Fold Stratified CV     internal validation on Western
                           │
                           ▼
                  Threshold Tuning         sweep 0.10-0.90
                                           optimise tumour recall >= 0.85
                           │
                           ▼
...
The default 0.50 threshold, optimised for Western training data, is entirely inappropriate for Indian samples. Tuning the SVM threshold to 0.10 improved macro F1 from 0.674 to 0.723. Any clinical deployment across population boundaries must treat the decision threshold as a population-specific hyperparameter — not a fixed constant.
...
# Experiment 2: Train on Western, test on Indian
python scripts/WestIndiSVM.py
python scripts/WestIndiXGB.py
```

Each script runs the full pipeline end-to-end and saves all outputs (CSVs + plots) to its designated results directory. Outputs include confusion matrices, ROC curves, precision-recall curves, SHAP beeswarm plots, threshold sweep charts, cross-validation bar charts, and score distribution histograms.
...
**Overlap Analysis** — genes appearing in both RF top-30 and SHAP top-30:

| Experiment | Method Comparison | Overlap |
|---|---|---|
| WestIndiSVM | RF top-30 ∩ SHAP top-30 | **14 genes** |
| WestIndiXGB | Gain top-30 ∩ SHAP top-30 | **10 genes** |

These overlapping genes are the strongest candidates for further biological validation — consistently prioritised by fundamentally different importance methods.
...
**Current Limitations**
- **Platform heterogeneity:** Western datasets use Illumina RNA-seq; the Indian dataset uses Affymetrix microarray. Platform-specific expression distributions may contribute to the generalization gap beyond true biological differences.
- **Small Indian cohort:** 232 samples limits statistical power and may not represent the full diversity of Indian breast cancer patients.
...
**Future Directions**
- **Domain adaptation:** CORAL, MMD minimization, or adversarial domain adaptation to explicitly align Western and Indian feature distributions
...
- **Larger Indian cohorts:** Collaborate with Indian cancer genomics initiatives (IndiGen, NCDIR) to expand training representation
- **Molecular subtype classification:** Extend to multi-class subtype prediction relevant to treatment decisions
- **Federated learning:** Collaborative model training across Indian hospital systems without centralizing patient data
